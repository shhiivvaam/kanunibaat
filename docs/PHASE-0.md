...
Vercel and Railway projects are **created in their dashboards**; this repo only ships **config hints**.

- **Vercel (web):** set **Root Directory** to `apps/web`, install `pnpm install`, build `cd ../.. && pnpm turbo build --filter=web` (or rely on Vercel’s monorepo detection). Set env vars from `.env.example`.
