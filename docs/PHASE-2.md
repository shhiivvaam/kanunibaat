...
This checklist maps to [Jurisly.md](../Jurisly.md) §6 (Phase 2) and the Phase 2 implementation plan. Status is tracked at a high level; adjust cells as work lands.
...
```bash
pnpm --filter @jurisly/database db:generate
pnpm --filter @jurisly/database db:migrate
```

For local iteration without migration files, `pnpm --filter @jurisly/database db:push` remains available (see [PHASE-0.md](./PHASE-0.md)).
