# Jurisly — Complete Platform Blueprint
...
Jurisly = India's legal infrastructure layer. Not just an app. A platform that becomes the word people say when they need legal help — like "Google it" or "Practo kar lo."
...
4. Shareable result card (with Jurisly watermark) — the viral mechanic
...
- New BNS/BNSS/BSA 2023 section mapper: "This IPC section is now BNS section \_\_\_"
...
FRONTEND WEB:     Next.js (App Router, RSC — see apps/web/package.json for major version)
MOBILE:           Expo / React Native (see apps/mobile/package.json for SDK line)
BACKEND:          NestJS 11 (apps/api) mounting the shared tRPC Express adapter
API LAYER:        tRPC (packages/trpc) + dedicated HTTP routes (webhooks, internal jobs, SSE)
DATABASE:         PostgreSQL + Drizzle ORM (packages/database)
CACHE:            Redis / Upstash selectively (e.g. OTP rate limiting on web; expand as needed)
SEARCH:           Meilisearch with Postgres fallback (@jurisly/search)
AUTH:             Better Auth — cookie sessions on web; Bearer session token for mobile/API (see session-resolve)
FILE STORAGE:     S3-compatible presigned uploads (@jurisly/storage)
REAL-TIME:        Consultation chat: PostgreSQL + SSE (`/sse/consultations/:id/messages`) with polling fallback; LiveKit env wired for A/V join tokens when configured
AI ORCHESTRATION: OpenAI / direct HTTP clients in routers (notice scan, research, QA, emergency guide)
EMAIL:            Resend (API app)
SMS / OTP:        MSG91 (India-first)
PAYMENTS:         Razorpay (+ webhooks in apps/api)
PUSH NOTIFS:      Expo Push Notifications + FCM pathways
VIDEO/AUDIO CALL: LiveKit token minting (consultations.liveKit.getToken) when LIVEKIT_* env is set
OCR:              Google Cloud Vision (implemented path); Tesseract fallback still product roadmap
AUDIT / METRICS:  audit_log table + writeAuditLog; structured external API + usage logs (logExternalApiCall, emitUsageMetric)
MONITORING:       Sentry (web, API, RN) + pino request logging; Grafana / OpenTelemetry as broader platform follow-up
DEPLOYMENT:       Web → Vercel (or host of choice), API → Railway/Fly.io-style Node, Mobile → EAS Build
CI/CD:            GitHub Actions + Turborepo remote caching (as configured in repo)
```
...
| DB        | Managed Postgres + Drizzle          | Supabase-only coupling | Core schema is Drizzle + Postgres; Supabase optional                            |
| Realtime  | SSE + PostgreSQL + polling fallback | Supabase Realtime      | Consultation chat uses SSE (`/sse/...`) today; Supabase Realtime still optional |
...
**Goal:** Public-facing website live. World can see Jurisly, understand the product, waitlist sign-up.
...
- In-app chat (SSE with polling fallback; LiveKit for A/V when configured)
...
```
PROMPT: Jurisly Website — Next.js 15 App Router

You are building the public-facing website for Jurisly, India's premier legal technology platform. This is a Next.js 15 application using App Router, TypeScript (strict), Tailwind CSS, and Framer Motion.

CONTEXT:
Jurisly is positioning itself as "Practo for legal services" in India — a platform that demystifies legal services for common people while giving lawyers powerful tools to manage their practice. The brand is trustworthy, approachable, Indian, and professional. Think clean, modern, and calm — not aggressive or salesy.
...
1. NAVIGATION (Navbar)
- Logo: "Jurisly" in Devanagari + Latin
...
a) HERO SECTION
- Headline (Hindi + English alternating animation): "Jurisly: legal help on your phone" / "Legal Help, Right in Your Pocket"
...
- Narrative: "The system is broken. Jurisly is not here to fix the courts — we're here to make sure you navigate them without fear."
...
```
PROMPT: Jurisly Mobile App — Expo React Native (SDK 52)

You are building the core Jurisly mobile app — the actual product where users get legal help and lawyers manage their practice. This is an Expo SDK 52 app using Expo Router (file-based routing), TypeScript strict, NativeWind (Tailwind for RN), and React Query.
...
```
PROMPT: Jurisly Backend API — NestJS 11

You are building the production-grade NestJS backend for Jurisly. This is the API powering both the Next.js web app and Expo mobile app.
...
```
PROMPT: Jurisly Shared Packages
...
```
PROMPT: Jurisly AI Services Layer
...
- Always include: "This is general information. Consult a verified Jurisly lawyer for advice specific to your situation."
...
# Email
RESEND_API_KEY=...
FROM_EMAIL=hello@tryjurisly.com
...
# App
NEXT_PUBLIC_API_URL=https://api.tryjurisly.com
NEXT_PUBLIC_APP_URL=https://www.tryjurisly.com
NODE_ENV=production
```
...
```
PROMPT: Initialize Jurisly Monorepo

Create a production-grade Turborepo monorepo for Jurisly with the following:
...
4. apps/mobile — Expo SDK 52:
   - Expo Router v4
   - NativeWind v4
   - app.json with correct bundle IDs (com.tryjurisly.app)
...
Ensure:
- All packages reference each other correctly (@jurisly/types, @jurisly/ui, etc.)
...
_This document is the complete technical and product specification for Jurisly. Execute phase by phase. Every phase builds on the previous. Start with Phase 0 (initialization), then Phase 1 (website), then Phase 2 (auth), and so on._

_Last updated: 2026 | Version 1.0_
