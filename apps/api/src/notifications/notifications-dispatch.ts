...
function tryConfigureWebPush(opts: {
  vapidPublicKey?: string | null;
  vapidPrivateKey?: string | null;
  vapidSubject?: string | null;
}): boolean {
  const pub = opts.vapidPublicKey?.trim();
  const priv = opts.vapidPrivateKey?.trim();
  const subject = opts.vapidSubject?.trim();
...
export function createNotificationsDispatchHandler(opts: {
  ...
  const webPushEnabled = tryConfigureWebPush({
