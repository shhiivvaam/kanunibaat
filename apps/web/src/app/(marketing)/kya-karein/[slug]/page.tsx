...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: `${scenario.titleHi} — General legal information and first steps on KanuniBaat.`,
  };
}

export default async function KyaKareinScenarioPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  if (!SLUGS.has(slug)) {
    notFound();
  }
