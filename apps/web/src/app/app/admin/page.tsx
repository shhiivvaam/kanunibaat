    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-[#57534E]" style={{ fontFamily: 'var(--font-body)', lineHeight: 1.6 }}>
          Review pending advocates, inspect uploaded documents metadata, then approve or reject. All actions are
          enforced again on the API (admin role required).
        </p>
      </div>
          ...
          {pending.data?.lawyers.map((row) => (
            ...
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-[#1C1917]">{row.displayName ?? row.email}</h2>
              ...
              <div className="mt-4 border-t border-[#F5F5F4] pt-4">
                <h3 className="text-xs font-semibold uppercase tracking-wide text-[#78716C]">Documents</h3>
                <ul className="mt-2 space-y-1 text-sm text-[#44403C]">
                  {row.documents.length === 0 ? (
                    <li className="text-[#A8A29E]">No document rows (unexpected for pending submission).</li>
                  ) : (
                    row.documents.map((d) => (
                      <li key={d.id}>
                        <span className="font-medium">{d.kind}</span> — {d.fileName} ({d.contentType},{' '}
