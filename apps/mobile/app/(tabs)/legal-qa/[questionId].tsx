    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {row ? (
        <View style={styles.card}>
          ...
          <View style={styles.row}>
            <Pressable
              style={styles.pill}
              onPress={() => void vote.mutateAsync({ questionId: row.id, value: 'up' })}
            >
              <Text>Up ({votes.up})</Text>
            </Pressable>
            <Pressable
              style={styles.pill}
              onPress={() => void vote.mutateAsync({ questionId: row.id, value: 'down' })}
            >
              <Text>Down ({votes.down})</Text>
            </Pressable>
            <Pressable
              style={styles.pill}
              onPress={() => void vote.mutateAsync({ questionId: row.id, value: null })}
            >
              <Text>Clear</Text>
            </Pressable>
            <View style={{ flex: 1 }} />
            <Pressable
              ...
              onPress={() => void ai.mutateAsync({ id: row.id, locale: 'en' })}
            >
      ...
      {aiPreview ? (
        <View style={[styles.card, { borderColor: '#fed7aa', backgroundColor: '#fff7ed' }]}>
          ...
          {aiPreview.summary ? (
            <Text style={[styles.body, { marginTop: 6 }]}>{aiPreview.summary}</Text>
          ) : null}
          ...
          {aiPreview.disclaimer ? (
            <Text style={[styles.meta, { marginTop: 8, color: '#7c2d12' }]}>
              {aiPreview.disclaimer}
            </Text>
          ) : null}
        </View>
      ...
      {canAnswer ? (
        <View style={styles.card}>
          ...
          <Pressable
            style={[
              styles.greenBtn,
              answer.isPending || answerBody.trim().length < 20 ? { opacity: 0.6 } : null,
            ]}
