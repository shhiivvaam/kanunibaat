...
import { lawyerAvailability, lawyerDocument, lawyerProfile, user, userProfile } from '@kb/database/schema';
import {
...
export const lawyerRouter = router({
  bootstrap: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const [existing] = await ctx.db.select().from(lawyerProfile).where(eq(lawyerProfile.userId, userId)).limit(1);
    if (!existing) {
  ...
  updateOnboarding: lawyerProcedure.input(updateOnboardingSchema).mutation(async ({ ctx, input }) => {
    const userId = ctx.authUserId;
    const [law] = await ctx.db.select().from(lawyerProfile).where(eq(lawyerProfile.userId, userId)).limit(1);
    if (!law) {
  ...
  requestDocumentUpload: lawyerProcedure.input(requestUploadSchema).mutation(async ({ ctx, input }) => {
    if (!ctx.s3Documents) {
      throw new TRPCError({
    ...
    if (law.verificationStatus === 'pending' || law.verificationStatus === 'verified') {
      throw new TRPCError({ code: 'CONFLICT', message: 'Documents cannot be changed in the current status.' });
    }
  ...
  confirmDocumentUpload: lawyerProcedure.input(confirmUploadSchema).mutation(async ({ ctx, input }) => {
    const userId = ctx.authUserId;
  ...
  submitForReview: lawyerProcedure.mutation(async ({ ctx }) => {
    ...
    if (law.verificationStatus !== 'draft' && law.verificationStatus !== 'rejected') {
      throw new TRPCError({ code: 'CONFLICT', message: 'Only draft or rejected profiles can be submitted.' });
    }
  ...
  setAvailability: lawyerProcedure.input(setAvailabilitySchema).mutation(async ({ ctx, input }) => {
    for (const s of input.slots) {
      if (s.endMinute <= s.startMinute) {
        throw new TRPCError({ code: 'BAD_REQUEST', message: 'Each slot must have endMinute greater than startMinute.' });
      }
    }
    const userId = ctx.authUserId;
